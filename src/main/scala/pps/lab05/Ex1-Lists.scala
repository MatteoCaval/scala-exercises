...
trait ListImplementation[A] extends List[A] {
  ...
  override def zipRight: List[(A, Int)] = {
    ...
    def zipWithAccumulator(list: List[A], currentIndex: Int): List[(A, Int)] = list match {
      case Cons(h, t) => (h, currentIndex) :: zipWithAccumulator(t, currentIndex + 1)
      case Nil() => Nil()
    }

    zipWithAccumulator(this, 0)
  }

  override def partition(pred: A => Boolean): (List[A], List[A]) = {
    this.foldRight(List[A](), List[A]())((elem, lists) => if (pred(elem)) (Cons(elem, lists._1), lists._2) else (lists._1, Cons(elem, lists._2)))
  }

  override def span(pred: A => Boolean): (List[A], List[A]) = {
    val a = this.foldLeft(List[A](), List[A](), true)((lists, elem) =>
      if (pred(elem) && lists._3)
        (Cons(elem, lists._1), lists._2, true)
      else
        (lists._1, Cons(elem, lists._2), false)
    )
    (a._1.reverse(), a._2.reverse())
  }
  ...
  override def reduce(op: (A, A) => A): A = this match {
    case Cons(h, t) => t.foldRight(h)(op)
    case _ => throw new UnsupportedOperationException
  }

  override def takeRight(n: Int): List[A] = {
    this.foldRight(n, List[A]())((elem, acc) => if (acc._1 > 0) (acc._1 - 1, Cons(elem, acc._2)) else (acc._1, acc._2))._2
  }

  override def collect[B](pf: PartialFunction[A, B]): List[B] = this.filter(pf.isDefinedAt).map(pf.apply)
