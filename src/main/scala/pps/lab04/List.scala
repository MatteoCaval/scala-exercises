...
object Lists extends App {
  ...
  object List {
    ...
    def fromSeq[A](vars: Seq[A]): List[A] = {
      var l: List[A] = Nil()
      vars.foreach(elem => l = Cons(elem, l))
      l
    }
    ...
    def contains[A](l: List[A])(item: A): Boolean = l match {
      case Cons(h, t) => if (h.equals(item)) true else contains(t)(item)
      case _ => false
    }
    ...
    def filterByFlatmap[A](l: List[A])(f: A => Boolean): List[A] = flatMap(l)(i => if (f(i)) Cons(i, Nil()) else Nil())


    def appendByFold[A](l1: List[A], l2: List[A]): List[A] = foldRight(l1)(l2)((elem, list) => Cons(elem, list))

    def length(l: List[_]): Int = foldRight(l)(0)((item, acc) => acc + 1)
  }
