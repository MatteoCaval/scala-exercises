...
import pps.lab04.Lists.List._
import pps.lab04.Lists._
// import custom List type (not the one in Scala stdlib)
...
object Student {
  def apply(name: String, year: Int = 2017): Student = StudentImp(name, year)
}
...
case class StudentImp(name: String, year: Int = 2017) extends Student {

  var courseList: List[Course] = Nil()

  override def enrolling(course: Course*): Unit =
    courseList = List.append(List.fromSeq(course), courseList)


  override def courses: List[String] = List.map(courseList)(_.name)

  override def hasTeacher(teacher: String): Boolean = List.contains(List.map(courseList)(_.teacher))(teacher)
}
